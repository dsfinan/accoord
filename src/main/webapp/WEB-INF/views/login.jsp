    ...
    $(document).ready(function(){
    	...
    	$.getJSON(rootPath+"/ajax/returnOrgs", function(orgs) {
    	    ...
    	    $.each(orgs, function(i, org) {
    	        $("<option />").attr("value", org.name).text(org.name).appendTo("#selectOrg");
    	    });
    	});
...
<div align="left" style="font: normal;font-size: 21px;padding-top: 50px;"><%=organization==null?"Organization":"Please Login" %></div>	
 ...
 <!--      <div class="error" id="error_client_username" ><%=I18N.getMsg("Give User Name")%></div>
  ...
  <!--     <div class="loginerror" id="error_client_password" ><%=I18N.getMsg("Give Password")%></div>
      ...
      <div class="login-label">Please select your organization.  If you do not see your organization, <a href="<%=request.getContextPath()%>/register">please register</a></div>
      <select id="selectOrg" name="organization"></select>
